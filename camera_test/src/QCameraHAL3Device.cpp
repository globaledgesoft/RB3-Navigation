...
void* doCapturePostProcess(void* data)
{
    ...
    printf("doCapturePostProcess - thread callback\n");
    while (true) {
        ...
        if (thData->msgQueue.empty()) {
            ...
            if(pthread_cond_timedwait(&thData->cond, &thData->mutex,&tv) != 0) {
                ...
                printf("doCapturePostProcess - thread Q empty\n");
                continue;
            }
        }
        ...
        printf("doCapturePostProcess - before calling post process\n");
        device->mCallback->CapturePostProcess(device->mCallback,&result);
        printf("doCapturePostProcess - after calling post process\n");
        // put the buffer back
...
int QCameraHAL3Device::ProcessOneCaptureRequest(int* requestNumberOfEachStream,int* frameNumber)
{
    ...
    printf("Inside ProcessOneCaptureRequest\n");
    ...
    if (res != 0) {
        ...
        printf("Error in process_capture_request\n");
...
void* doprocessCaptureRequest(void* data)
{
    ...
    printf("inside doprocessCaptureRequest\n");
    printf("doprocessCaptureRequest req no is %d\n", thData->requestNumber[DEPTH_STREAM]);
    ...
    while (!thData->stopped) { //need repeat and has not trigger out until stopped
        ...
        */
        ...
        printf("doprocessCaptureRequest mStreams size is %d\n", (int)device->mStreams.size());
        ...
        printf("inside doprocessCaptureRequest %d\n", hasRequest);
        ...
        if (res != 0) {
            ...
            printf("Error in ProcessOneCaptureRequest\n");
            return nullptr;
        }
    ...
    CameraThreadData* resultThread)
{
    ...
    printf("processCaptureRequestOn for debug\n");
    ...
    printf("req no is %d", requestThread->requestNumber[DEPTH_STREAM]);
